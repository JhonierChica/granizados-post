...
import { EditIcon, DeleteIcon, ToggleIcon, WalletIcon, InfoIcon, TruckIcon } from '../../../components/common/Icons';
import { ORDER_STATUS, USER_ROLES } from '../../../utils/constants';
import { useAuth } from '../../../context/AuthContext';
import { normalizeProfileCode } from '../../../utils/roles';
import type { Delivery, Order, StatusBadge } from '../../../types';
...
interface DeliveryCardProps {
  ...
  onEdit: (delivery: Delivery, order: Order | null) => void;
...
const DeliveryCard: React.FC<DeliveryCardProps> = ({
  ...
  return (
    <Card className="h-full flex flex-col group border-b-8 border-b-primary/5 hover:border-b-primary/40 transition-all duration-500">
      ...
      <div className="flex flex-wrap justify-between items-start gap-2 mb-4">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            <span className="text-lg sm:text-2xl font-black tracking-tighter text-foreground">ENVÍO</span>
            <span className="font-mono text-base sm:text-xl font-black text-primary bg-primary/5 px-1.5 rounded-md">#{delivery.id}</span>
          </div>
          <p className="text-[9px] text-muted-foreground font-black uppercase tracking-wider opacity-60">
            Pedido #{delivery.orderId}
            {delivery.createdAt
              ? (() => {
                  const [, time] = delivery.createdAt.split('T');
      ...
      {/* Info: Cliente + Dirección en una fila */}
      <div className="flex gap-2 mb-3">
        <div className="flex-1 bg-muted/30 px-2.5 py-2 rounded-lg border border-muted/50 min-w-0">
          ...
          <p className="font-black text-foreground text-[11px] sm:text-xs truncate">{order?.clientName || 'Cliente General'}</p>
        </div>
        <div className="flex-1 bg-muted/30 px-2.5 py-2 rounded-lg border border-muted/50 min-w-0">
          <span className="text-[7px] sm:text-[8px] font-black text-muted-foreground uppercase tracking-widest block">Contacto</span>
          <p className="font-black text-foreground text-[11px] sm:text-xs truncate">{order?.clientPhone || '--'}</p>
        </div>
      </div>
      <div className="flex gap-2 mb-3">
        <div className="flex-1 bg-muted/30 px-2.5 py-2 rounded-lg border border-muted/50 min-w-0">
          <span className="text-[7px] sm:text-[8px] font-black text-muted-foreground uppercase tracking-widest block">Dirección</span>
          <p className="font-black text-foreground text-[11px] sm:text-xs truncate">{order?.clientAddress || 'Sin dirección'}</p>
        </div>
      </div>
      ...
      <div className="flex gap-1.5 pt-4 mt-4 border-t border-dashed border-muted/60">
        ...
        {delivery.status === 'DELIVERED' && canCollectPayment && (
          <button
            onClick={() => onInitiatePayment(delivery)}
            className="flex-1 flex items-center justify-center gap-1.5 h-10 bg-primary text-white rounded-lg font-black text-[8px] uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-primary/20"
          >
            <WalletIcon size={14} />
            COBRAR
          </button>
